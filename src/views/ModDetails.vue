...
onMounted(async () => {
	...
	titleFrag.value = currentMod.value.name
    Storage.set("current_mod", mod_id);
});
...
const builds = computed(() =>
    currentMod.value["mod-builds"].sort((a, b) => b.build_id - a.build_id)
);
...
<template>
    <div id="details">
        <div class="container" v-if="currentMod">
            ...
            <div class="info">
                ...
                <div class="mini-info">
                    ...
                    <button v-if="!updateAvailable" class="play">
                        <Icon icon="material-symbols:play-arrow" />Запустить
                    </button>
                    <button v-else @click="updateMod" class="update">
                        <Icon
                            icon="material-symbols:sync-problem-rounded"
                        />Обновить
                    </button>
                </div>
            </div>
            <div class="builds">
                <div class="build" v-for="build in builds">
                    ...
                    <TorrentDownload
                        v-if="
                            processedBuilds.value &&
                            (processedBuilds.value[
                                build.build_id.toString()
                            ] !== undefined ||
                                enableDownload === build.build_id)
                        "
                        :done="processedBuilds.value[build.build_id.toString()]"
                        :magnet="build.download_url"
                        :mod="currentMod"
                        :build="build"
                    />
                </div>
